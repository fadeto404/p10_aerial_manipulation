function [tau, f, n_hat, f_uav, n_uav] = rne_aerial_manipulator(q, q_dot, q_ddot, m, r, p, I, R, om0, dom0, v0, dv0, g_vec)
...
% f(:,end) = m(end)*g_vec;
F = sym(zeros(3, n+1));     % forces acting on each link at its CoM
...
dv(:,1) = dv0+g_vec;        % acceleration of base frame
...
for i=2:n+1
    ...
    F(:,i-1) = simplify(m(i) * (dv_c(:,i-1)));

    % Torque acting on link CoM
    N_hat(:,i-1) = simplify(I(:,:,i) * dom(:,i) + ...
...
for i=n:-1:1
    ...
    if (i==1)
        f_uav = simplify(R(:,:,i) * f(:,i) + m(i)*dv0);
        n_uav = simplify(I(:,:,i)*dom0 + ...
                R(:,:,i) * n_hat(:,i) + ...
                cross(p(:,i), (R(:,:,i)) * f(:,i)));
    end
end