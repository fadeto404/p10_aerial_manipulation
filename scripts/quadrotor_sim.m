...
I2 = eye(3); % Link 2 inertia [?]quat_pr
...
% TODO: EXPLAIN TO ME WHY I NEED TO ROTATE THE ANGULAR VELOCITY OR I WILL
% FUCKING KILL YOU. 
% EXPLAIN WHETHER OR NOT I CAN ROTATE THE ANGULAR ACCELERATION INTO THE 
% BODY FRAME OR I WILL FUCKING KILL YOU. 
% EXPLAIN IT WITHOUT JUST STATING THE EQUATIONS OR I WILL FUCKING KILL YOU. 
% EXPLAIN WHY THE FUCK DOES IT NOT CHANGE ANYTHING IF I APPLY THE ROTATION 
% OR NOT IN THE OTHER SIMULATION OR I WILL FUCKING KILL YOU.
% EXPLAIN TO ME WHY, WHEN I ROTATE THE ANGULAR ACCELERATION HERE, IT
% MATCHES WITH IF I SWAPPED THE ORDER OF THE EULER ANGLES IN THE OTHER SIM
% OR I WILL FUCKING KILL YOU.
x_sym = [q_sym; x_B; y_B; z_B; Omega_sym; x_B_dot; y_B_dot; z_B_dot];
x_sym_dot = [0.5*quat_prod_ham(x_sym(1:4), x_sym(8:10));
             rod_qv_rotate((x_sym(1:4)), x_sym(11:13));
             I0\(-cross(x_sym(8:10), I0*x_sym(8:10)) + Gamma_sym);
             1/m0 * (F_total_sym + rod_qv_rotate(quat_conj(x_sym(1:4)), F_g) - cross(x_sym(8:10), m0*x_sym(11:13)))
             ];
x_sym_dot = [0.5*quat_prod_ham(x_sym(1:4), x_sym(8:10));
             x_sym(11:13);
             I0\(-cross(x_sym(8:10), I0*x_sym(8:10)) + Gamma_sym);
...
%% Simulation setup
% Control sequence
omega_rotor = [sqrt(0.26*m0*g/k_t); sqrt(0.26*m0*g/k_t); sqrt(0.25*m0*g/k_t); sqrt(0.25*m0*g/k_t)].*ones(4, N_t);
...
p_B0 = [0; 0; 0.3]; % Initial position
x0 = [q0; p_B0; zeros(6,1)]; % Initial state [q1 q2 q3 q4 x_q y_q z_q omega_x omega_y omega_z dx_q dy_q dz_q]
...
%% Animation of quadrotor
% Allow a little time for figures to settle, else Matlab will animate in 
% the previous figures.
pause(1); 
quadrotor_animate(x_sim(5:7,:), x_sim(1:4,:), dx_sim(11:13,:), S, t_s, 3)
